...
/**
 * Build consistent headers for all outbound requests.
 * - Attach i18n header when missing
 * - Auth headers should be added by @delon/auth interceptors (ITokenService/JWTInterceptor/SimpleInterceptor)
 */
...
/**
 * Single-source auth/header contract (skeleton).
 * All interceptors/refresh flows should rely on this helper to avoid drift.
 * Token headers handled by @delon/auth interceptors; this hook keeps locale/header shape.
 */
export function buildAuthHeaderContract(headers?: HttpHeaders): Record<string, string> {
  return getAdditionalHeaders(headers);
}
