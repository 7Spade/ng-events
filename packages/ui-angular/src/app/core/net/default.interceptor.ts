...
import { ReThrowHttpError, checkStatus, buildAuthHeaderContract, toLogin } from './helper';
...
export const defaultInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  // Enforce single-source header contract (token由 @delon/auth 處理)
  const newReq = req.clone({ url, setHeaders: buildAuthHeaderContract(req.headers) });
