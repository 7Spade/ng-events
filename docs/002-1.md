## 002-1 Kernel Quick Reference（與 001 同步）

🧭 為什麼存在

- 舊筆記與 001 內容重複，保留為速查版並指向 001 作為唯一真實來源。
- 若兩邊不一致，以 001 為準；修改時請同步兩份或只改 001。
...
閉環、可 replay、可 debug。詳細解釋與不變量請見 001。

### 使用提醒

- [ ] 修改前先讀 001，避免分歧。
- [ ] 引擎層不帶業務語意；Saga 純函數；Compensation 只看 context。
- [ ] 發現矛盾時，優先修正 001 並在此更新鏈接。
